...
class GUI:
    ...
    def on_start_click(self) -> None:
        """
        Callback function triggered when the start button is clicked.

        This function retrieves user input from a text field, handles potential exceptions, and starts
        ...
        try:
            ...
            self.thread_manager.start_threads(threads_range, self.output_text)
    ...
    def on_stop_click(self) -> None:
        """
        Same as `on_start_click`, with only difference that this method stops corresponding threads.
